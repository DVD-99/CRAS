...
class ChromaVectorStore(VectorStoreInterface):
    ...
    def add_documents(self, ids: List[UUID], embeddings: List[np.ndarray], metadatas: List[dict]):
        ...
        logger.info(f"Added {len(ids)} documents with metadata to ChromaDB collection.")

    def search(self, query_embedding: np.ndarray, top_k: int, where_filter: Optional[dict] = None) -> List[tuple[UUID, float]]:
        query_params = {"query_embeddings": [query_embedding.tolist()], "n_results": top_k}
        if where_filter:
            query_params["where"] = where_filter
        results = self.collection.query(**query_params)
        if not results['ids'] or not results['distances']: return []
